...
function handleRuleError(type, decl, ruleName, subst_0, subst_1) {
...
function getRootMacro(base, decl) {
    var _a;
    let chain = [base];
    let cur = (_a = decl.options[base]) === null || _a === void 0 ? void 0 : _a.macroFor;
    for (let i = 0; i < MAX_REFERENCE_DEPTH; i++) {
        ...
        if (!(cur in decl.options)) {
            ...
            handleRuleError(3, decl, base, cur, decl.options[cur].macroFor);
            return;
        }
        chain.push(cur);
...
function demacroRule(rule, decl) {
    if (!(rule === null || rule === void 0 ? void 0 : rule.macroFor))
        return rule;
...
function resolveReference(base, decl) {
    ...
    for (let i = 0, cur = base; i < MAX_REFERENCE_DEPTH; i++) {
        ...
        if ((rule === null || rule === void 0 ? void 0 : rule.reference) === undefined) {
            break;
        }
            ...
            handleRuleError(3, decl, base, cur, rule.reference);
            break;
        }
        if (cur)
            refChain.push(cur);
